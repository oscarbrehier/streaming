		...
		<div className="absolute top-0 left-0 h-[85vh] w-full overflow-hidden">
			...
			<MovieOverview movie={data}>
				...
				<div className="flex space-x-4">
					...
					<Link
						href={`/watch/${data.id}`}
						className="capitalize bg-neutral-200 text-black text-lg py-2 px-6 rounded-3xl cursor-pointer flex items-center sm:space-x-4"
					>
					...
					<Link
						href={`/movie/${data.id}`}
						className={cn(
							"capitalize text-lg py-2 px-6 rounded-3xl cursor-pointer flex items-center sm:space-x-4",
							glass()
						)}
					>
